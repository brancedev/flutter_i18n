...
class FlutterModule {
  FlutterModule(Directory source, [Directory? output])
      : assert(source.existsSync(), 'The source folder does not exist.'),
        ...
        output = output != null && output.existsSync()
            ? output
            : Directory(join(source.path, 'generated'))
  ...
  Map<String, Map<String, String>> get values =>
      files.asMap().map((_, File it) => MapEntry<String, Map<String, String>>(
          _languageFromPath(it.path), getLanguageData(it)));

  List<String> get languages => files //
      .map((File it) =>
          basenameWithoutExtension(it.path).split('_').skip(1).join('_'))
      .toList();
    ...
    final StringBuffer buffer = StringBuffer()
      ...
      ..writeAll(
          languages.map<String>(
              (String language) => "part 'strings_$language.dart';"),
          '\n')
      ..writeln()
      ..writeln()
  ...
  void createFiles() {
    final File generatedFile =
        File(join(output.path, '$_generatedFileName.dart'));
    ...
    for (String language in values.keys) {
      final String data =
          createLanguageClass(language, englishKeys, values[language]);
      final File languageFile =
          File(join(output.path, 'strings_$language.dart'));
  ...
  File arbFileForLanguage(String language) {
    return File(
        setExtension(absolute(source.path, 'strings_$language'), '.arb'));
  }

  File dartFileForLanguage(String language) {
    return File(
        setExtension(absolute(output.path, 'strings_$language'), '.dart'));
  }
  ...
  String createSClass(Map<String, String> englishData) {
    ...
    buffer
      ...
      ..writeln('  const S();')
      ..writeln()
      ..writeln('  static S current = const S();')
      ..writeln()
      ...
      ..writeln()
      ..writeAll(parametrized.map<String>(
          (String key) => createParametrizedMethod(key, englishData[key]!)))
      ..writeln()
      ..writeAll(pluralsKeys.map<String>((String key) =>
          createPluralMethod(key, pluralsQuantities[key], englishData)))
      ..writeln('}');

    return buffer.toString();
  }

  String createLanguageClass(String language, List<String> englishKeys,
    ...
    final List<String> parametrized = keys
        .where((String key) => languageData[key]!.contains(r'$'))
        .toList()
          ..sort();
    keys
      ..removeWhere(parametrized.contains)
    ...
    final String textDirection =
        _rtl.contains(language.split('_')[0]) ? 'rtl' : 'ltr';
    ...
    buffer //
      ...
      ..writeln()
      ..writeAll(keys.map<String>((String key) =>
          createValuesMethod(key, languageData[key], isOverride: true)))
      ..writeln()
      ..writeAll(parametrized.map<String>((String key) =>
          createParametrizedMethod(key, languageData[key]!, isOverride: true)))
      ..writeln()
      ..writeAll(pluralsKeys.map<String>((String key) => createPluralMethod(
          key, pluralsQuantities[key], languageData,
          isOverride: true)))
      ..writeln('}');
  ...
  String createDelegateClass(List<String> languages) {
    ...
    buffer
      ...
      ..writeln()
      ..writeln(
          r'  LocaleListResolutionCallback listResolution({Locale? fallback, bool withCountry = true}) {')
      ..writeln(
          r'    return (List<Locale>? locales, Iterable<Locale> supported) {')
      ..writeln(r'      if (locales == null || locales.isEmpty) {')
      ..writeln(r'        return fallback ?? supported.first;')
      ..writeln(r'      } else {')
      ..writeln(
          r'        return _resolve(locales.first, fallback, supported, withCountry);')
      ..writeln(r'      }')
      ..writeln(r'    };')
      ..writeln(r'  }')
      ..writeln()
      ..writeln(
          r'  LocaleResolutionCallback resolution({Locale? fallback, bool withCountry = true}) {')
      ..writeln(r'    return (Locale? locale, Iterable<Locale> supported) {')
      ..writeln(
          r'      return _resolve(locale, fallback, supported, withCountry);')
      ..writeln(r'    };')
      ..writeln(r'  }')
      ..writeln()
      ..writeln(r'  @override')
    ...
    buffer
      ...
      ..writeln(r'  @override')
      ..writeln(
          r'  bool isSupported(Locale locale) => _isSupported(locale, true);')
      ..writeln()
      ..writeln(r'  @override')
      ..writeln(
          r'  bool shouldReload(GeneratedLocalizationsDelegate old) => false;')
      ..writeln()
      ..writeln(
          r'  /// Internal method to resolve a locale from a list of locales.')
      ..writeln(
          r'  Locale? _resolve(Locale? locale, Locale? fallback, Iterable<Locale> supported, bool withCountry) {')
      ..writeln(
          r'    if (locale == null || !_isSupported(locale, withCountry)) {')
      ..writeln(r'      return fallback ?? supported.first;')
      ..writeln(r'    }')
      ..writeln()
      ..writeln(
          r'    final Locale languageLocale = Locale(locale.languageCode, "");')
      ...
      ..writeln(r'    } else {')
      ..writeln(
          r'      final Locale fallbackLocale = fallback ?? supported.first;')
      ..writeln(r'      return fallbackLocale;')
      ..writeln(r'    }')
      ..writeln(r'  }')
      ..writeln()
      ..writeln(
          r'  /// Returns true if the specified locale is supported, false otherwise.')
      ...
      ..writeln(
          r'        if (supportedLocale.languageCode != locale.languageCode) {')
      ..writeln(r'          continue;')
      ..writeln(r'        }')
      ..writeln()
      ..writeln(r'        // If country code matches, return this locale.')
      ..writeln(
          r'        if (supportedLocale.countryCode == locale.countryCode) {')
      ..writeln(r'          return true;')
      ..writeln(r'        }')
      ..writeln()
      ..writeln(
          r'        // If no country requirement is requested, check if this locale has no country.')
      ..writeln(
  ...
  PluralsResult findPluralsKeys(List<String> keys) {
    ...
    for (int i = 0; i < keys.length; i++) {
      ...
      if (isPlural) {
        ...
        pluralsQuantities[actualKey] =
            (pluralsQuantities[actualKey] ?? <String>[]) //
    ...
    pluralsQuantities.removeWhere((String actualId, List<String> quantities) {
      if (!quantities.any((String quantity) => quantity == 'other')) {
        for (int i = 0; i < quantities.length; i++) {
          ...
          pluralKeys.removeWhere((String key) =>
              key.toLowerCase() == '$actualId$quantity'.toLowerCase());
        }
  ...
  String createValuesMethod(String key, String? value,
  ...
  String createParametrizedMethod(String key, String value,
      {bool isOverride = false}) {
    final StringBuffer buffer = _createBuffer(isOverride);
  ...
  String createPluralMethod(
      String key, List<String>? quantities, Map<String, String> values,
      {bool isOverride = false}) {
    final StringBuffer buffer = _createBuffer(isOverride);
    ...
    for (int i = 0; i < _pluralEnding.length; i++) {
      ...
      final String _key = values.keys.firstWhere((String possibleKey) =>
          possibleKey.toLowerCase() == '$key$quantity'.toLowerCase());
  ...
  String _extractOtherParameterName(String key, Map<String, String> values) {
    final String otherValueKey = values.keys.firstWhere((String _key) =>
        _key.startsWith(key) && _key.toLowerCase().endsWith('other'));
    final String otherValueValue = values[otherValueKey]!;

    if (parameterRegExp.hasMatch(otherValueValue)) {
      return _normalizeParameter(
          parameterRegExp.firstMatch(otherValueValue)!.group(0)!);
    } else {
      return 'param';
    }
  }
  ...
  String _languageFromPath(String path) =>
      basenameWithoutExtension(path).split('_').skip(1).join('_');

  bool _isArb(File it) =>
      extension(it.path) == '.arb' &&
      basenameWithoutExtension(it.path).startsWith('strings_');
  ...
  int _sortQuantities(String a, String b) =>
      _pluralEndingSortOrder[a]!.compareTo(_pluralEndingSortOrder[b]!);
  ...
  static const List<String> _rtl = <String>[
    'ar',
    'dv',
    'fa',
    'ha',
